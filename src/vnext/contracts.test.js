...
test('immutable presentation is not created from incomplete preflight or the first execution step', () => {
  ...
  const ready = { ...run, status: 'awaiting_confirmation', preview: { status: 'complete', evidence: {graph_version:'test'} } };
  expect(projectionForRun(ready)).toEqual({ run_id: 'r', phase: 'preview' });
  const executing = { ...ready, status: 'running', stage: 'querying_graph', evidence: { graph_version:'test', steps: [{ id: 'step1' }] } };
