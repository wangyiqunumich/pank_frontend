...
function IntermediatePage({ onContinue }) {
  ...
  if (error && vnextEnabled) {
    ...
    return <Container disableGutters maxWidth={false} className="pk-search-state-wrap">
      <Box className="pk-state-panel" role={nativeSearchError ? 'alert' : 'status'} aria-labelledby="pk-search-state-title" aria-describedby="pk-search-state-description">
        <Box component="img" src={notRelevant} alt="" aria-hidden="true" sx={{ width: 160, height: 160, objectFit: 'contain' }} />
        <Typography component="h1" id="pk-search-state-title" className="pk-state-title">{title}</Typography>
        <Typography component="p" id="pk-search-state-description" className="pk-state-description">{message}</Typography>
        <Box className="pk-state-panel__actions">
          <Button variant="contained" onClick={() => setSearchAttempt(attempt => attempt + 1)}>Retry search</Button>
